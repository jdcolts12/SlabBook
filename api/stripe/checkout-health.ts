...
import { getServiceRole, getSupabaseUrl } from '../../server/supabaseAdmin'

/**
 * GET — whether checkout *can* run server-side (booleans only, no secrets).
 * Open in browser: /api/stripe/checkout-health
 */
export default function handler (req: VercelRequest, res: VercelResponse) {
  ...
  const supabaseService = Boolean(getServiceRole())
  const appOrigin = Boolean(process.env.APP_ORIGIN?.trim() ?? process.env.VERCEL_URL?.trim())

  const ready =
    stripeSecret && pricePro && supabaseUrl && supabaseService
